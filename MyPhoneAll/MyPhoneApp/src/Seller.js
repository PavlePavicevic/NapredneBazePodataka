...
import PhoneList from "./components/Seller/PhoneList";
...
export default function Seller({ type, reloadHeader }) {
  ...
  const getInfo = async () => {
    const response = await fetch(
      "http://localhost:5100/api/Seller/GetSeller/" + id,
      {
        credentials: "include",
      }
    );
    ...
    <Container component="main" sx={{ pt: 3 }}>
        ...
        <Grid item xs={12} md={10}>
          ...
          <Box sx={{ display: type === "public" ? "none" : "flex", mt: 1 }}>
            <EditSeller
              ...
              Phones={info.phoneIds}
              Reviews={info.reviews}
              update={update}
            />
            <Button variant="contained" sx={{ml:2}} onClick={()=>{navigate("/AddSeller/"+id)}}>
              Add Phone
            </Button>
          </Box>
        </Grid>
      ...
      <Box>
        ...
        <TabPanel value={value} index={0}>
          <PhoneList PhoneIds={info.phoneIds} />
        </TabPanel>
